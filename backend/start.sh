...
until PGPASSWORD="$POSTGRES_PASSWORD" timeout 10 psql -h postgres -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "SELECT 1;" > /dev/null 2>&1; do
  ...
  if [ $RETRY_COUNT -ge $MAX_RETRIES ]; then
    ...
    if PGPASSWORD="$POSTGRES_PASSWORD" timeout 10 psql -h postgres -U "$POSTGRES_USER" -d postgres -c "SELECT 1;" > /dev/null 2>&1; then
